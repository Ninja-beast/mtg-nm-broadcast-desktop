...
db.exec(`UPDATE matches SET updated_at = created_at WHERE updated_at = '' OR updated_at IS NULL`);
...
export function getUserRoles(userId: number): string[] {
  const rows = db.prepare(`SELECT role FROM user_roles WHERE user_id = ? ORDER BY role`).all(userId) as { role: string }[];
 ...
 */
export function setUserRoles(userId: number, roles: string[]): void {
  const unique = Array.from(new Set(roles.filter(Boolean)));
  if (unique.length === 0) throw new Error("En konto ma ha minst en rolle.");
  ...
  const tx = db.transaction(() => {
    ...
    db.prepare(`UPDATE users SET role = ? WHERE id = ?`).run(unique[0], userId);
  });
  tx();
}
 ...
 */
export function dedupeMatches(tournamentId: number): number {
  const rows = db
  ...
  for (const list of groups.values()) {
    ...
    const survivor = [...list].sort((a, b) => {
      ...
      if (aHasMelee !== bHasMelee) return bHasMelee - aHasMelee;
      return b.id - a.id;
    })[0];
    ...
    for (const loser of list) {
      ...
      db.prepare(`UPDATE judge_entries SET match_id = ? WHERE match_id = ?`).run(survivor.id, loser.id);
      db.prepare(`UPDATE broadcast_state SET bo3_match_id = ? WHERE bo3_match_id = ?`).run(survivor.id, loser.id);
      db.prepare(`UPDATE broadcast_state SET bo5_match_id = ? WHERE bo5_match_id = ?`).run(survivor.id, loser.id);
 ...
 */
export async function runBackup(): Promise<{ filename: string; path: string; sizeBytes: number; createdAt: string }> {
...
export function listBackups(): { filename: string; path: string; sizeBytes: number; createdAt: string }[] {
  const dir = resolveBackupsDir();
  return fs
    .readdirSync(dir)
...
export function getLogSettings(): { level: string; retentionDays: number } {
  return {
...
export function setLogSettings(level: string, retentionDays: number) {
  setSetting("log_level", level);
  setSetting("log_retention_days", String(retentionDays));
}

export function appLog(level: "debug" | "info" | "warn" | "error", message: string): void {
  const { level: minLevel, retentionDays } = getLogSettings();
  if ((LOG_LEVEL_RANK[level] ?? 1) < (LOG_LEVEL_RANK[minLevel] ?? 1)) return;

  db.prepare(`INSERT INTO app_logs (level, message) VALUES (?, ?)`).run(level, message);
  db.prepare(`DELETE FROM app_logs WHERE created_at < datetime('now', ?)`).run(`-${retentionDays} days`);

  const consoleFn = level === "error" ? console.error : level === "warn" ? console.warn : console.log;
  consoleFn(`[${level.toUpperCase()}] ${message}`);
}

export function getLogs(minLevel: string, limit: number): { id: number; level: string; message: string; created_at: string }[] {
  ...
  return db
    ...
    .all(...allowedLevels, limit) as any[];
}
