...
export type AuthSession = { token: string; username: string; roles: Role[] };
export type UserAccount = { id: number; username: string; role: Role; roles: Role[]; created_at: string };
...
export async function getCurrentSession(): Promise<{ username: string; roles: Role[] } | null> {
  const res = await fetch(`${API_BASE}/auth/me`);
  if (!res.ok) return null;
  return res.json();
}

export async function login(username: string, password: string): Promise<{ ok: boolean; token?: string; username?: string; roles?: Role[]; error?: string }> {
...
export async function createUser(username: string, password: string, roles: Role[]): Promise<{ ok: boolean; error?: string; id?: number }> {
  const res = await fetch(`${API_BASE}/users`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
...
export async function setUserRoles(id: number, roles: Role[]): Promise<void> {
