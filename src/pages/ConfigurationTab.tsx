...
import { useObsSettings, useMeleeSettings, useMeleeLog } from "../hooks/useMelee";
import { useMetaScene, useMetaKeycards } from "../hooks/useContent";
import { useThemesList, useActiveTheme } from "../hooks/useTheme";
import { SCENE_LABELS, SCENE_KEYS_ORDERED } from "../config/scenes";

function ObsConnectionSettings() {
  const [obsStatus, reloadObsStatus] = useObsSettings();

  const [obsHost, setObsHost] = useState("localhost");
  const [obsPort, setObsPort] = useState("4455");
  const [obsPassword, setObsPassword] = useState("");
  const [placeholderScene, setPlaceholderScene] = useState("Placeholder");
  const [connecting, setConnecting] = useState(false);

  useEffect(() => {
    if (obsStatus?.host) setObsHost(obsStatus.host);
    if (obsStatus?.port) setObsPort(obsStatus.port);
    if (obsStatus?.placeholderScene) setPlaceholderScene(obsStatus.placeholderScene);
  }, [obsStatus?.host, obsStatus?.port, obsStatus?.placeholderScene]);

  async function saveObsSettings() {
    await fetch(`${API_BASE}/settings/obs`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ host: obsHost, port: obsPort, password: obsPassword, placeholderScene })
    });
    setObsPassword("");
    reloadObsStatus();
  }
  ...
  return (
    <div style={{ background: "var(--raised)", padding: 16, borderRadius: 10, maxWidth: 560, marginBottom: 20 }}>
      ...
      <div style={{ marginTop: 16, paddingTop: 12, borderTop: "1px solid var(--border)" }}>
        <div style={{ fontSize: 13, fontWeight: 700, marginBottom: 4 }}>Fallback-scene for Preview</div>
        <div style={{ fontSize: 12, opacity: 0.7, marginBottom: 8 }}>
          Hvis en knapp i Broadcast peker pa en scene som ikke finnes i OBS, brukes denne scenen i Preview i stedet for a bare feile stille. Ma finnes i OBS med akkurat dette navnet.
        </div>
        <input style={{ ...inputStyle, width: 200 }} value={placeholderScene} onChange={(e) => setPlaceholderScene(e.target.value)} placeholder="Placeholder" />
        <button onClick={saveObsSettings} style={{ padding: "6px 14px", borderRadius: 5, border: "1px solid var(--border)", background: "var(--raised)", color: "var(--text-heading)" }}>
          Lagre
        </button>
      </div>
    </div>
...
function ObsSceneNameOverrides() {
  const [settings, reloadSettings] = useObsSettings();
...
function MeleeSyncSection() {
  const [settings, reloadSettings] = useMeleeSettings();
  const [log, reloadLog] = useMeleeLog();
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <h3>Melee-synk</h3>
      <div style={{ background: "var(--raised)", padding: 16, borderRadius: 10, maxWidth: 480, marginBottom: 16 }}>
      ...
      <div style={{ background: "var(--raised)", padding: 12, borderRadius: 10, maxHeight: 300, overflowY: "auto" }}>
        <div style={{ fontSize: 12, opacity: 0.6, marginBottom: 6 }}>Siste synk-forsok</div>
...
function MetaKeycardsSection() {
  const [meta] = useMetaScene();
  const [overrides, reloadOverrides] = useMetaKeycards();
...
function ThemeSection() {
  const [themesList, reloadThemesList] = useThemesList();
  ...
  return (
    <div>
      <div style={{ marginBottom: 16 }}>
        ...
        <div style={{ fontSize: 13, color: "var(--text-muted)", marginTop: 4 }}>OBS connection, Melee sync, meta keycards and overlay theme - all in one place</div>
      </div>
